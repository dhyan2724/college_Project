...
router.post(
  "/",
  authenticateToken,
  authorizeRoles("student", "phd_scholar", "dissertation_student", "faculty"),
  async (req, res) => {
    try {
      ...
      const issuedItemData = {
        ...
        issuedToId:
          req.body.issuedToId || req.body.issuedTo || req.body.issuedToId,
      ...
      if (!issuedItemData.issuedToId) {
        return res
          .status(400)
          .json({ message: "issuedToId is required and cannot be null" });
      }
      ...
      switch (itemType) {
        case "Chemical":
          ...
          if (inventoryItem) {
            ...
            if (
              Number(inventoryItem.availableWeight) < Number(weightToDeduct)
            ) {
              return res.status(400).json({
                message: `Insufficient chemical available. Available: ${inventoryItem.availableWeight}g, Requested: ${weightToDeduct}g`,
              });
            }
            inventoryItem.availableWeight =
              Number(inventoryItem.availableWeight) - Number(weightToDeduct);
            await Chemical.updateById(inventoryItem.id, {
              availableWeight: inventoryItem.availableWeight,
            });
          }
          break;
        ...
        case "Glassware":
          ...
          if (inventoryItem) {
            ...
            inventoryItem.availableQuantity =
              ...
              Number(quantityToDeduct);
            await Glassware.updateById(inventoryItem.id, {
              availableQuantity: inventoryItem.availableQuantity,
            });
            console.log(
              "✅ Glassware inventory updated. New available quantity:",
              inventoryItem.availableQuantity,
            );
          }
          break;
        ...
        case "Plasticware":
          ...
          if (inventoryItem) {
            ...
            inventoryItem.availableQuantity =
              Number(inventoryItem.availableQuantity) -
              Number(quantityToDeduct);
            await Plasticware.updateById(inventoryItem.id, {
              availableQuantity: inventoryItem.availableQuantity,
            });
            console.log(
              "✅ Plasticware inventory updated. New available quantity:",
              inventoryItem.availableQuantity,
            );
          }
          break;
        ...
        case "Instrument":
          ...
          if (inventoryItem) {
            ...
            inventoryItem.availableQuantity =
              Number(inventoryItem.availableQuantity) - 1;
            await Instrument.updateById(inventoryItem.id, {
              availableQuantity: inventoryItem.availableQuantity,
            });
            console.log(
              "✅ Instrument inventory updated. New available quantity:",
              inventoryItem.availableQuantity,
            );
          }
          break;
      ...
      if (req.body.pendingRequestId) {
        ...
        await PendingRequest.updateById(req.body.pendingRequestId, {
          status: "approved",
        });
      }
...
router.patch(
  "/:id",
  authenticateToken,
  authorizeRoles("admin", "faculty", "master_admin"),
  async (req, res) => {
    try {
      const issuedItem = await IssuedItem.findById(req.params.id);
      if (!issuedItem)
      ...
      if (req.body.status === "returned" && previousStatus !== "returned") {
        console.log("🔄 Marking item as returned:", {
        ...
        switch (itemType) {
          case "Chemical":
            ...
            if (inventoryItem) {
              ...
              inventoryItem.availableWeight =
                Number(inventoryItem.availableWeight) + Number(weightToRestore);
              await Chemical.updateById(inventoryItem.id, {
                availableWeight: inventoryItem.availableWeight,
              });
              console.log(
                "✅ Chemical inventory restored. New available weight:",
                inventoryItem.availableWeight,
              );
            }
            break;
          ...
          case "Glassware":
            ...
            if (inventoryItem) {
              ...
              inventoryItem.availableQuantity =
                Number(inventoryItem.availableQuantity) +
                Number(quantityToRestore);
              await Glassware.updateById(inventoryItem.id, {
                availableQuantity: inventoryItem.availableQuantity,
              });
              console.log(
                "✅ Glassware inventory restored. New available quantity:",
                inventoryItem.availableQuantity,
              );
            }
            break;
          ...
          case "Plasticware":
            ...
            if (inventoryItem) {
              ...
              console.log(
                "✅ Plasticware inventory restored. New available quantity:",
                inventoryItem.availableQuantity,
              );
            }
            break;
          ...
          case "Instrument":
            ...
            if (inventoryItem) {
              ...
              console.log(
                "✅ Instrument inventory restored. New available quantity:",
                inventoryItem.availableQuantity,
              );
            }
            break;
        ...
        try {
          ...
          studentName = issuedItem.issuedTo;
        }
