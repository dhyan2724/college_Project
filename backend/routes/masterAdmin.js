  ...
  async (req, res) => {
    try {
      ...
      const validRoles = [
        ...
        "student",
        "phd_scholar",
        "dissertation_student",
      ];
      if (!validRoles.includes(role)) {
        return res.status(400).json({
          message:
            "Invalid role. Must be one of: admin, faculty, student, phd_scholar, dissertation_student",
        });
      }
      ...
      // For admin and faculty roles, academic fields must be null to satisfy DB constraints
      const isStudentRole = [
        "student",
        "phd_scholar",
        "dissertation_student",
  ...
  async (req, res) => {
    try {
      ...
      if (user.role === "master_admin") {
        return res
          .status(403)
          .json({ message: "Cannot delete master admin account" });
      }
    ...
    try {
      ...
      const statistics = {
        ...
        byRole: {
          ...
          dissertation_student: users.filter(
            (u) => u.role === "dissertation_student",
          ).length,
        },
